...
- **Go** toolchain (untuk CLI `formspec`).
- Binary **`formspec`** tersedia di `PATH` (di mesin ini:
  `C:\Users\prima\go\bin\formspec.exe`).
...
#    di :8443. Untuk kerja lokal TIDAK dipakai; `formspec dev` membaca spec
#    langsung dari folder.
formspec apply --spec spec
```

### Konfigurasi `formspec-app.yaml`

```yaml
spec: spec                    # folder manifest
dsn: sqlite:.formspec/arisan.db  # SQLite untuk dev
...
2. Jalankan `formspec validate --spec spec` sampai bersih.
3. `formspec dev` — server hot-reload setiap ada perubahan spec (log:
...
Database SQLite dev berada di `.formspec/arisan.db`. Untuk reset bersih:

```bash
# Hentikan server, lalu hapus file DB
Remove-Item .formspec\arisan.db -Force
# Restart `formspec dev` — engine akan membuat skema dari nol
```
...
| Mode | `formspec dev` | `dsn: postgres://...` |
...
koneksi transaksi aksi. Sudah dipatch lokal (module cache) dan `formspec.exe`
di-rebuild.
...
| Path | Isi |
|------|-----|
| `formspec-app.yaml` | Konfigurasi CLI (bukan manifest) |

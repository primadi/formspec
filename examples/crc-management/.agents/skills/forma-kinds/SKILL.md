---
name: forma-kinds
description: Catalog of all Forma resource kinds — Entity, Service, App, Module, Page, Form, Table, Dashboard, Widget, Report, Wizard, Kanban, Timeline, Calendar, Listing, ApprovalInbox, NotificationCenter, Print, Theme, Config, Migration, Subscription, Workflow, Api, Webhook, Mockup, KindDefinition, Integrator, Renderer, PersistBackend, Environment, Policy, Datastore. Use when the user asks about Forma kinds, needs to choose the right kind for a task, asks how to declare a YAML manifest, or mentions specific kinds by name. Also use when creating a new Forma app to understand which kinds to declare.
metadata:
  version: "1.0"
  source: docs/spec/platform/03-kind-system.md + schemas/kinds/
---

# Forma Kinds — Complete Catalog

Every Forma resource is declared as a YAML manifest with a `kind` field.
...
**Not to be confused** with `forma-app.yaml` (which is CLI dev/serve config).
...
Uses `visible_when`, `readonly_when`, `required_when`, `compute` expressions (FormaExpr).
...
- **Entity characteristics are mutually exclusive.** `forma apply` rejects more than one.
...
- **`transaction` characteristic REQUIRES** an explicit `transaction_date` field — `forma apply` rejects if missing.

...
func TestOTCSale_EndToEnd(t *testing.T) {
	...
	status, env = do(t, handler, "POST", "/default/_ui/entity/pharmacy/otc-sale", map[string]any{
	...
	status, env = do(t, handler, "POST", "/default/_ui/entity/pharmacy/otc-sale/"+saleID+"/sell", nil)
	if status != http.StatusOK {
		t.Fatalf("sell: status %d, body %+v", status, env)
	}

	status, env = do(t, handler, "GET", "/default/_ui/entity/pharmacy/otc-sale/"+saleID, nil)
	if status != http.StatusOK {
...
func TestOTCSale_Cancel(t *testing.T) {
	...
	status, env = do(t, handler, "POST", "/default/_ui/entity/pharmacy/otc-sale/"+saleID+"/cancel", nil)
	if status != http.StatusOK {
...
func TestOTCSale_SellRejectsEmptyItems(t *testing.T) {
	...
	status, _ = do(t, handler, "POST", "/default/_ui/entity/pharmacy/otc-sale/"+saleID+"/sell", nil)
	if status == http.StatusOK {

...
| `shift`           | `(branch_id, cashier_id)` **WHERE `status = 'open'`** | **unique (partial)** | `indexes:` dengan `where:` (D6)      |
...
| Satu shift terbuka per kasir per outlet       | Partial unique index (`indexes[].where`)                                     |

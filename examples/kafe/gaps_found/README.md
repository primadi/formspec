...
**Update 2026-09-22 (audit sisa prosa):** ringkasan "sisa terbuka" di atas sudah
kedaluwarsa dan dikoreksi. Status final setelah re-verifikasi langsung:

| Status | Gap | Bukti |
| --- | --- | --- |
| ✅ **Tertutup** | #16 (7.3 — `byQualified`), #17 (7.4 — `TimelineRenderer` pakai `useRealtime`), #21 (8.3 `validateDanglingRefs` + 8.7 `impl.ref`), #32 (4.4 — keunikan = urusan DB, `indexes:` parsial), #34 (4.7 — `HookDecl.Uses` ada di `pkg/spec/entity.go:1381`), #13 (4.1 — valuasi moving-average berjalan), #14 (6.5 — keputusan tertulis, opsi b) | diverifikasi ulang di kode 2026-09-22 |
| 🔴 **Terbuka** | #15 (6.3 ⏸️ Control Plane), #42 (6.4 ⏸️ keputusan bahasa spec), #45 (sisa → 2.15 ⏸️) | ledger `TODO.md` Fase 6 + 2.15 |
| 🟡 **Sebagian → item bernomor** | #1/#3 (kartu meja QR → 2.15), #4/#5/#8 (adopsi → 10.4, 10.3) | `TODO.md` Fase 10 |

Sisa pekerjaan non-blocker aplikasi kafe kini terkumpul di **`TODO.md` Fase 10**
(10.1–10.5) — sebelumnya hanya prosa di bawah item `[x]` dan tidak bisa di-grep.
...
| 13  | Inventory vertical: tanpa valuasi (FIFO/HPP), tanpa stock opname, `transfer` rusak → ✅ **Ditutup 2026-09-20 (TODO 4.1)**: valuasi moving-average berjalan (proyeksi `stock-level` terisi; diuji bisa gagal), stock opname + waste + purchase-order ada di module kafe sendiri. Sisa adopsi: landed cost (**10.1 ⏸️**) + integrasi purchase → jurnal (**10.2 ⏸️**) — keputusan produk, bukan gap engine                                                                                                                                                                                                                                                                                        | **BLOCKER** (HPP/margin)          | ✅ Pasti                                                                         |
| 14  | Vertical `purchase` tidak dibuat → ✅ **Diputuskan 2026-09-20 (TODO 6.5, opsi b)**: kafe memodelkan purchase di module sendiri (`supplier`, `purchase-order`, `receive-goods` → `stock-movement`) — keputusan tertulis, bukan gap terbuka. Sisa adopsi di **10.1/10.2 ⏸️**                                                                                                                                                                                                                                                                                                                | **HIGH**                          | ✅ Pasti                                                                         |
...
| 21  | Validator tidak menangkap referensi menggantung (`App.spec.modules`, menu `view:`, `impl.ref` ke file `.star`) → ✅ **Ditutup 2026-09-20 (TODO 8.3 + 8.7)**: `validateDanglingRefs` menolak module tak dikenal, `view:` tak dikenal (termasuk nested), dan `impl.ref` yang tidak ada; marker `# GAP-21` di `spec/` ditutup 9.5. Status final: tertutup | **HIGH**                          | ✅ Terverifikasi |
...
| 32  | ✅ **Ditutup 2026-09-18 (TODO 4.4)** — Guard keunikan di script tidak atomik — butuh `ctx.lock`, jadi reimplementasi UNIQUE yang lebih rapuh dari constraint-nya. Jawabannya: keunikan adalah **urusan database** (`indexes:`, termasuk parsial) dan berlaku untuk semua jalur tulis; guard script hanya lapis kedua untuk pesan ramah. Terverifikasi: dua index unik ada di DB nyata, `TestMigrationRunner_UniqueIndexRejectsDuplicates` hijau | **HIGH**                          | ✅ Terverifikasi |
...
| 34  | ✅ **Ditutup 2026-09-18 (TODO 4.7)** — `HookDecl` tidak punya `uses` → akses script hook tidak terlihat di consent footprint. Kini `HookDecl.Uses *UsesDecl` ada (`pkg/spec/entity.go:1381`) dan kafe mendeklarasikan `uses: {primitives: [db]}` di hook-nya | **MEDIUM**                        | ✅ Terverifikasi |
...
| 45  | 🟡 **Sebagian 2026-09-14** — izin per-entity ✅ (S3/`public_entities`), penyelesaian oleh kasir ✅ (#52 rute lifecycle, #44 `order` lifecycle-free), dan klaim kepemilikan tamu ✅ (#45 tertutup lewat `order.guest_token` + grant `scope` di 2.2). **Sisa satu:** QR meja langsung-ke-sesi (`table-session.qr_token` → sesi) = item **2.15 ⏸️**, yang butuh `context.source` resolve-by-token | 🔴 **BLOCKER**                    | ✅ Terverifikasi runtime |

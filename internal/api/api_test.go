...
func setupAPIEnv(t *testing.T) (*entity.Registry, db.DB) {
	t.Helper()

	dir := t.TempDir()
	d, err := db.OpenSQLite(filepath.Join(dir, "api_test.db"), nil)
	if err != nil {
	...
	// Manually register entities with expose: rest
	reg.LoadEntities() // no-op on empty dir

	// We register manually bypassing LoadEntities (which wants files)
	// by directly calling SyncSchema with our entities

	// But since Registry doesn't expose Register/Add method,
	// we need to use migration runner directly + our loader.

	// For now, use the approach of creating entities through migration
	...
	orderSpec := spec.EntitySpec{
		...
		Expose: []spec.ExposeConfig{
			{Type: spec.ProtocolREST},
			// No Actions filter → all default (list, find, create, update, but NOT delete)
		},
	}

	// Private entity: no expose
	secretMeta := spec.Metadata{Name: "secret", Module: "billing"}
	secretSpec := spec.EntitySpec{
		Version: "v1",
		Fields: []spec.Field{
			{Name: "data", Type: spec.FieldString},
		},
		// No Expose → private
	}

	// Build entity migrations manually and run them
	// We'll re-register via the registry by creating a wrapper
	// For now, just test the route generation logic directly
	_ = customerMeta
	_ = customerSpec
	_ = orderMeta
	_ = orderSpec

...
//
// Design doc: docs/renderers/shadcn-shell/01-architecture.md §4,
// docs/renderers/shadcn-shell/02-derivation-engine.md.
package ui
...
//
// Every navigable kind must appear here. This list, the client's buildRoutes
// (renderers/react-shadcn/src/shell/router.tsx), routeExists/navigationPrefix
// below in meta.go, and viewKinds in cmd/formspec/validate_dangling.go are FOUR
// copies of the same convention, and they must agree. Listing was missing from
// this one alone: `formspec validate` accepted `view: product-catalog` (it is
// in viewKinds) while app.Resolve failed with "view not found" — an App that
// validates green but mounts nothing. registry_test.go now pins the whole
// table.
func (r *Registry) ResolveViewRoute(module, name string) (string, error) {
	...
	if e, ok := r.Listings[name]; ok && e.Module == module {
		return "/listing/" + name, nil
	}

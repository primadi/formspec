...
// NaturalKeyEntry states who supplies a natural key — the record's business
// identifier. The distinction is about UI and authority, not storage: whichever
// mode is declared, the engine still guarantees uniqueness.
//
// The three modes exist because the same field type serves opposite needs:
// `journal-entry.number` must never be typed by a person, `account.code` may be
// filled in whenever someone gets to it, and `order.number` is normally minted
// but a cashier sometimes has a paper number to use instead.
// @schema {title: "Natural Key Entry", description: "Who supplies the natural key: `auto_generated` (engine only), `user_entry` (a person), `auto_generated_if_empty` (engine mints it unless a value is supplied).", enum: ["auto_generated", "user_entry", "auto_generated_if_empty"]}
type NaturalKeyEntry string

const (
	// NaturalKeyEntryAutoGenerated — the engine mints the value and a
	// caller-supplied value is IGNORED. The key is not an input at all.
	NaturalKeyEntryAutoGenerated NaturalKeyEntry = "auto_generated"
	// NaturalKeyEntryUserEntry — a person supplies it. A `natural_key_rule`
	// may still be declared for format/validation purposes, but the engine
	// never mints a value on a plain insert.
	NaturalKeyEntryUserEntry NaturalKeyEntry = "user_entry"
	// NaturalKeyEntryAutoGeneratedIfEmpty — the engine mints a value only when
	// none is supplied, so the user may override the generated one. This is
	// the historical `strategy: sequence` behaviour, and what
	// EntityStore.generateNaturalKeys already implements.
	NaturalKeyEntryAutoGeneratedIfEmpty NaturalKeyEntry = "auto_generated_if_empty"
)

// ResolveNaturalKeyEntry returns the entry mode in effect for a field,
// applying the convention that keeps declarations written before this property
// existed meaningful:
//
...
//
// The rule-backed default is deliberately auto_generated_if_empty rather than
// auto_generated: `generateNaturalKeys` already leaves a caller-supplied value
// untouched, and two production manifests (`order.number`,
// `purchase-order.number`) depend on exactly that — a cashier overriding the
// minted number with the paper one in hand.
func ResolveNaturalKeyEntry(f *Field) NaturalKeyEntry {
	if f.NaturalKeyEntry != "" {
		return f.NaturalKeyEntry
	}
	if f.NaturalKeyRule == nil || f.NaturalKeyRule.Strategy == "custom" {
		return NaturalKeyEntryUserEntry
	}
	return NaturalKeyEntryAutoGeneratedIfEmpty
}

// MintsNaturalKey reports whether the ENGINE may generate this field's value.
// It answers "who is allowed to author the key", which is a different question
// from whether a generator exists — see HasNaturalKeyGenerator.
func MintsNaturalKey(f *Field) bool {
	switch ResolveNaturalKeyEntry(f) {
	case NaturalKeyEntryAutoGenerated, NaturalKeyEntryAutoGeneratedIfEmpty:
		return true
	}
	return false
}

// HasNaturalKeyGenerator reports whether the field declares something the engine
// can actually generate a value WITH.
//
// It is deliberately separate from MintsNaturalKey: the mode says who is
// *allowed* to supply the key, while this says whether a generator exists. A
// field declaring `auto_generated` with no rule answers true to the first and
// false to this one — which is exactly the contradiction the validator refuses,
// and the reason a mode-only check could not catch it.
func HasNaturalKeyGenerator(f *Field) bool {
	return f.NaturalKeyRule != nil && f.NaturalKeyRule.Strategy != "custom"
}
...
type Field struct {
	...
	// Unique declares a cross-record uniqueness constraint (unique per tenant).
	// Implied by NaturalKey: ValidateEntitySpec sets it rather than making authors
	// write both, because storage already keys one unique index off
	// `Unique || NaturalKey` — two sources for one fact is how the core manifests
	// drifted apart.`unique: false` is a contradiction in that case and refused.
	Unique bool `yaml:"unique,omitempty" json:"unique,omitempty"`
	Index  bool `yaml:"index,omitempty" json:"index,omitempty"`
	// NaturalKey marks the field as the record's business identifier.
	// At most one per entity. The engine guarantees exactly one thing about it:
	// UNIQUENESS (implied — no need to write `unique: true`).
	//
	// Presence is the author's choice, not the engine's. Two modes:
	//   - entry otomatis: a `natural_key_rule` (usually `strategy: sequence`)
	//     mints the value on insert;
	//   - diisi user: no rule, and `required` decides whether a person must
	//     enter it or may leave it blank (a chart-of-accounts code, filled in
	//     when someone needs it).
	//
	// A blank value on an optional natural key means "not set" rather than "a
	// value", so the uniqueness index skips it (see jsonb-persist/ddl.go).
	...
	// NaturalKeyEntry states who supplies the key: the engine, a person, or the engine unless a person supplies one (see the NaturalKeyEntry enum).
	// The distinction is about UI and authority, not storage — a generated number
	// must not be asked for, while a code the user types must not be silently
	// overwritten. Absent resolves by convention (see ResolveNaturalKeyEntry) so
	// every declaration written before this property existed keeps its meaning.
	NaturalKeyEntry NaturalKeyEntry `yaml:"natural_key_entry,omitempty" json:"natural_key_entry,omitempty"`
	...
	// Multiple states how many of the declared `options` this field holds: a
	// set (`true`) or a single value (`false`). It is the *cardinality* of the
	// data, so it belongs here rather than in a Form's `widget:` — otherwise
	// the same field reads as multi-select in one form and single in another,
	// and a table/detail/filter has no way to know which it is.
	//
	// A pointer because "not declared" must stay distinguishable from `false`:
	// `json` and `string` can hold either shape, so `options` on them is
	// ambiguous without this, and the validator refuses to guess. On scalar
	// types an absent value simply means `false`.
	// @schema {description: "How many of the declared `options` this field holds: `true` = a set (array / comma-separated list), `false` = one value. Required when `options` is declared on `json` or `string`, because those types can hold either shape."}
	...
	// uniqueSet records whether the YAML actually carried `unique:` as opposed to
	// omitting it. A plain bool cannot tell "absent" from "explicitly false", and
	// that distinction is load-bearing for natural keys: `natural_key: true`
	// implies uniqueness, so writing `unique: false` is a contradiction the
	// validator must refuse, while saying nothing at all is the normal case.
	// Unexported so it stays out of JSON, `%+v` debugging output, and the
	// generated JSON Schema (the schema walker skips unexported fields).
	uniqueSet bool
}

// UnmarshalYAML decodes a Field and records which boolean keys were present.
//
// It exists purely for `unique` presence detection (see the unexported field
// above). Decoding goes through `plain`, a defined type with the same tags but
// an empty method set — so the call below cannot re-enter this method.
	...
	*f = Field(p)
	...
	if value.Kind == yaml.MappingNode {
		for i := 0; i+1 < len(value.Content); i += 2 {
			if value.Content[i].Value == "unique" {
				f.uniqueSet = true
			}
		}
	}
...
// isAmbiguousOptionContainer reports whether a field type can hold either a
// single value or a set of values, so `options` on it cannot be interpreted
// without an explicit `multiple` (05-field-types.md §1.1.1).
//
// `json` can store `1` or `[1, 2]`; `string` can store `senin` or
// `senin,selasa`. Nothing in the type says which was meant, and guessing is how
// the cardinality decision ended up living in a Form's `widget:` instead of in
// the data.
func isAmbiguousOptionContainer(t FieldType) bool {
	return t == FieldJSON || t == FieldString
}

// isOptionScalarType reports whether a field type can carry a labelled option
// set at all: a scalar (single value) or one of the two ambiguous containers.
//
// Everything else keeps its own contract — `enum` has `enum_values` (and a DB
// CHECK constraint), the structured types (`money`, `file`, `relation`, `child`)
// render through their own widgets, and `text`/`richtext` are free-form prose
// rather than a choice.
func isOptionScalarType(t FieldType) bool {
	switch t {
	case FieldJSON, FieldString,
		FieldInteger, FieldDecimal, FieldPercent,
		FieldDate, FieldDateTime, FieldTime,
		FieldBoolean, FieldUUID:
		return true
	}
	return false
}

// FieldIsMultiple reports the declared cardinality of a field: `true` for a set,
// `false` for one value. Absent means `false` for a scalar field; for `json`/
// `string` the caller gets a non-nil error, because those types hold either
// shape and an absent declaration is exactly the ambiguity `multiple` exists to
// remove.
//
// Exported so `formspec check` and the renderer parity tests read cardinality
// by the same rule as the validator — one vocabulary, one decision.
func FieldIsMultiple(f *Field) (bool, error) {
	if f.Multiple != nil {
		return *f.Multiple, nil
	}
	if isAmbiguousOptionContainer(f.Type) {
		return false, fmt.Errorf("field %q: `multiple` is required on a %s field with `options` — it holds either one value or a set, and nothing in the type says which (add `multiple: true` for a set of choices, `multiple: false` for one)", f.Name, f.Type)
	}
	return false, nil
}

// WidgetCardinalityMismatch reports a Form `widget:` that contradicts the field's
// declared cardinality, so the same field cannot be a tag picker in one form and
// a single select in another.
//
// The rule lives here, next to the cardinality it enforces, because it is read by
// two callers that must never disagree: `formspec check` (deploy-time gate over
// Form manifests) and the renderer derivation (`formWidget`). When a widget is
// not in the cardinality-sensitive set the check passes — most widgets
// (`moneyinput`, `fileinput`, …) say nothing about cardinality.
func WidgetCardinalityMismatch(f *Field, w FormWidget) error {
	switch w {
	case WidgetSelectMultiTag:
		if len(f.Options) == 0 && len(f.EnumValues) == 0 {
			return nil // no declared set — nothing to be multiple of
		}
		multi, err := FieldIsMultiple(f)
		if err != nil {
			return err
		}
		if !multi {
			return fmt.Errorf("field %q: `widget: select-multi-tag` needs a field that holds a set, but the Entity declares %s (add `multiple: true` on the field, or use a single-value widget)", f.Name, cardinalityWord(f))
		}
	case WidgetSelect, WidgetRadioGroup, WidgetCombobox:
		if len(f.Options) == 0 {
			return nil // `enum_values`-driven selects are single by nature
		}
		multi, err := FieldIsMultiple(f)
		if err != nil {
			return err
		...
		if multi {
			return fmt.Errorf("field %q: `widget: %s` picks one value, but the Entity declares the field a set (`multiple: true`) — use `select-multi-tag`, or set `multiple: false` if one value is intended", f.Name, w)
		}
	}
	return nil
}

// cardinalityWord renders the declared cardinality for an error message,
// keeping the actionable half ("declare `multiple:`") readable.
func cardinalityWord(f *Field) string {
	if f.Multiple == nil {
		return fmt.Sprintf("`multiple: %v` by default for %s", false, f.Type)
	}
	return fmt.Sprintf("`multiple: %v`", *f.Multiple)
}

// validateFieldOptions checks a field's `options:` declaration: its shape (each
// option is a storable, uniquely-valued scalar) and its cardinality (how many of
// the options the field holds).
//
// Validated tightly for the same reason as `unit`: a declaration the renderer
// cannot honour reads as "the widget is broken".
func validateFieldOptions(f *Field) error {
	if err := validateFieldOptionsShape(f); err != nil {
		return err
	}
	return validateFieldOptionsCardinality(f)
}

// validateFieldOptionsShape checks the individual options: each needs a
// storable, non-duplicate scalar value. It also refuses `options` on a field
// type that has no way to act on them at all.
//
...
//   - a field type that can never hold a choice set (see isOptionScalarType).
//     For `enum` the message points at `enum_values`: that contract already
//     carries a DB CHECK constraint, and adding a second source of truth for
//     the same set is how the two drift apart. Captions for enum values are
//     therefore still a gap, tracked as todo 5.10.23 — `options` covers every
//     *other* scalar type since `Field.Multiple` landed (5.10.19).
func validateFieldOptionsShape(f *Field) error {
	if len(f.Options) == 0 {
		return nil
	}
	if !isOptionScalarType(f.Type) {
		hint := ""
		switch f.Type {
		case FieldEnum:
			hint = " — an enum declares its values in `enum_values` (with a database CHECK constraint); captions for enum values are tracked separately"
		case FieldMoney, FieldFile, FieldAttachment, FieldRelation, FieldChild:
			hint = " — this type renders through its own widget, not a choice list"
		case FieldText, FieldRichText:
			hint = " — free-form prose is not a fixed choice set"
		}
		return fmt.Errorf("field %q: `options` is not valid on a %q field%s", f.Name, f.Type, hint)
	}
...
// validateFieldOptionsCardinality enforces the type × cardinality matrix
// (05-field-types.md §1.1.1). `multiple` is the *data*'s shape, so it is a
// property of the Entity — not of one Form's `widget:` — and the two must agree:
//
//   - `json`/`string` hold either shape, so `multiple` is REQUIRED there;
//   - a scalar type stores one value, so `multiple: true` on it is refused
//     rather than silently accepted (a claim nothing can act on);
//   - `multiple` without `options` describes nothing.
func validateFieldOptionsCardinality(f *Field) error {
	if f.Multiple == nil {
		// Absent is fine for a scalar without `options`, and for a plain
		// `json`/`string` field that declares no choice set (free-form JSON is
		// still free-form).
		if len(f.Options) > 0 {
			_, err := FieldIsMultiple(f)
			return err
		}
		return nil
	}
	...
	if len(f.Options) == 0 {
		return fmt.Errorf("field %q: `multiple` only means something with `options` — declare the choices, or drop `multiple`", f.Name)
	}

	if *f.Multiple && !isAmbiguousOptionContainer(f.Type) {
		return fmt.Errorf("field %q: `multiple: true` is not valid on a %q field — it stores a single value; use `json` to hold a set, or `multiple: false` for one choice", f.Name, f.Type)
	}
...
func ValidateEntitySpec(d *EntitySpec) error {
	...
	//
	// `natural_key` is the record's business identifier, and its THREE entry
	// modes state who supplies it (see NaturalKeyEntry): the engine alone
	// (`auto_generated`), a person (`user_entry`), or the engine unless a person
	// supplies one (`auto_generated_if_empty`, the historical
	// `strategy: sequence` behaviour that `order.number` and
	// `purchase-order.number` depend on).
	//
	// The engine guarantees exactly ONE thing in every mode: UNIQUENESS. It
	// therefore implies `unique` — storage already keys its unique index off
	// `Unique || NaturalKey` (jsonb-persist/ddl.go), so requiring the author to
	// write `unique: true` as well was a second source of truth for one fact,
	// which is how the core manifests (subscription, period-closing) came to
	// sit in a state that failed to load.
	//
	// It does NOT imply `required`. Presence is a product decision: `account.code`
	// is filled by a seed, `journal-entry.number` is generated, and a user-filled
	// code may legitimately stay empty until someone needs it. Asserting an
	// obligation the author did not declare is how an earlier version of this
	// check broke 12 manifests.
	//
	// An explicit `unique: false` IS refused, because it denies the one guarantee
	// the declaration makes. Telling it apart from omission needs a presence flag
	// (a plain bool decodes both to the same value) — hence `uniqueSet` on Field.
	{
		var nkField string
		for i := range d.Fields {
			f := &d.Fields[i]
			...
			if f.uniqueSet && !f.Unique {
				return fmt.Errorf("field %q: `natural_key: true` implies uniqueness, so `unique: false` contradicts it — remove the `unique` key, or drop `natural_key`", f.Name)
			}

			// Entry mode: validate the declaration, then keep it resolved on the
			// spec so every consumer (store, meta bundle, form derivation) reads
			// one answer instead of re-deriving the convention.
			switch f.NaturalKeyEntry {
			case "", NaturalKeyEntryAutoGenerated, NaturalKeyEntryUserEntry, NaturalKeyEntryAutoGeneratedIfEmpty:
			default:
				return fmt.Errorf("field %q: natural_key_entry must be auto_generated, user_entry, or auto_generated_if_empty, got %q", f.Name, f.NaturalKeyEntry)
			}
			entry := ResolveNaturalKeyEntry(f)
			f.NaturalKeyEntry = entry
			switch entry {
			case NaturalKeyEntryAutoGenerated:
				// Nothing to mint with would make "engine only" a promise the
				// engine cannot keep, and the value would simply be absent.
				if !HasNaturalKeyGenerator(f) {
					return fmt.Errorf("field %q: natural_key_entry=auto_generated needs a `natural_key_rule` to generate with — add one, or use user_entry", f.Name)
				}
			case NaturalKeyEntryAutoGeneratedIfEmpty, NaturalKeyEntryUserEntry:
				// Both tolerate a user-supplied value; the difference is only
				// whether the engine fills an empty one, which the presence or
				// absence of a rule already answers.
			}

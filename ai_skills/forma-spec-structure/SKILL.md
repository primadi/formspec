---
name: forma-spec-structure
description: Navigates the Forma specification documentation structure. Use when the user asks where to find spec docs, how the spec folder is organized, which spec file covers a topic (entity, kinds, frontend, backend, platform), how to read the Forma docs, or what the Forma project layout should be. Covers the 3 sub-folders (platform/, backend/, frontend/), doc status lifecycle (Outline→Draft→Final), and the contract-vs-renderer principle.
metadata:
  version: "1.0"
  source: docs/spec/
---

# Forma Spec Structure

Context for AI coding agents working on Forma apps — where to find the
...
| `frontend/` | Any Shell & visual renderer: 4-tier hierarchy, VisualSpecKind, Renderer, Spec Resolution API, kind catalog, FormaExpr | `frontend/README.md` |

## Platform Docs (Cross-Cutting)

| File | Covers |
|------|--------|
| `01-overview.md` | What Forma is, contract-vs-renderer, personas, scope |
...
| `08-project-layout.md` | **Project folder structure** — THE reference for scaffolding. Covers `spec/`, `app/`, `forma-app.yaml`, module organization by entity characteristic |
...
| `error-glossary.yaml` | Canonical `FORMA.*` error codes |
...
4. **Use the kinds catalog** (`forma-kinds` skill) to find which kind maps to which spec file
...
- **`forma-app.yaml` is dev/serve config for the CLI — NOT a `kind: Config` manifest.** It points the engine to spec path, DSN, and runtime settings.

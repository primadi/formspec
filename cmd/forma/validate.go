// Command forma validate — dry-run validation of a Forma spec tree.
//
// Two layers, both reported:
//  1. Engine loader (internal/manifest) — the ground truth of what
//     `forma dev` / `forma apply` would accept: YAML parse errors and
//     deep Entity validation (expose, lifecycle, relations, state machine,
//     transaction_date, reserved fields, ...).
//  2. JSON Schema (schemas/forma.schema.json) — the contract for ALL kinds
...
// Usage:
//
//	forma validate [--spec <path>] [--schema <dir>] [--no-schema]
//
...
import (
	...
	formaroot "github.com/primadi/forma"
...
func runValidate(args []string) {
	...
	if fs.NArg() > 0 {
		fmt.Fprintf(os.Stderr, "forma validate: unexpected argument %q\n", fs.Arg(0))
		os.Exit(2)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "forma validate: load error: %v\n", err)
		os.Exit(2)
	}
	...
	if !*noSchema {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "forma validate: schema: %v\n", err)
			os.Exit(2)
		}
	}
...
// body) with the shared $defs from forma.schema.json, wrapped in the universal
...
func newKindSchemaCompiler(schemaDir string) (*kindSchemaCompiler, error) {
	...
	var err error
	if schemaDir != "" {
		rootData, err = os.ReadFile(schemaDir + "/forma.schema.json")
	} else {
		rootData, err = formaroot.SchemasFS.ReadFile("schemas/forma.schema.json")
	}
	if err != nil {
		return nil, fmt.Errorf("read forma.schema.json: %w", err)
	}
	var root map[string]any
	if err := json.Unmarshal(rootData, &root); err != nil {
		return nil, fmt.Errorf("decode forma.schema.json: %w", err)
	}
	defs, _ := root["$defs"].(map[string]any)
	if defs == nil {
		return nil, fmt.Errorf("forma.schema.json: missing $defs")
	}
	c.rootDefs = defs
	return c, nil
}
...
func (c *kindSchemaCompiler) readKindSchema(kind string) ([]byte, error) {
	...
	if c.kindDir != "" {
		...
		data, err = formaroot.SchemasFS.ReadFile("schemas/kinds/" + kind + ".schema.json")
	}
	if err != nil {

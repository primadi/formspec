// forma-operator is the Forma CRD controller — it translates Forma CRDs
...
//
// See docs/runtimes/03-forma-operator.md and
...
import (
	...
	formacontroller "github.com/primadi/forma/internal/operator/controller"
	"github.com/primadi/forma/internal/operator/report"
)
...
func init() {
	...
	utilruntime.Must(formav1alpha1.AddToScheme(scheme))
}
...
func main() {
	...
	flag.StringVar(&resourceImage, "resource-image", "formahub/forma-resource:latest", "Generic forma-resource image for workspace Deployments (pin a version in production)")
	...
	mgrOptions := ctrl.Options{
		...
		LeaderElectionID:       "forma-operator.forma.dev",
	}
	if namespace != "" {
	...
	wsReconciler := &formacontroller.WorkspaceReconciler{
	...
	if err := (&formacontroller.DatastoreReconciler{Client: mgr.GetClient()}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to set up controller", "controller", "Datastore")
		os.Exit(1)
	}

	if err := (&formacontroller.ResourceClaimReconciler{
		Client:                      mgr.GetClient(),
	...
	// reconcilers (docs/runtimes/03-forma-operator.md §3.2).
	...
	setupLog.Info("starting forma-operator",

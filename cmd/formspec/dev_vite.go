...
// findWebDir locates the renderers/react-shadcn/ directory (for Vite HMR via --dev-ui).
// Walks up from CWD looking for renderers/react-shadcn/package.json, then falls back to
...
func findWebDir() (string, error) {
	// 1. Walk up from CWD looking for renderers/react-shadcn/package.json
	dir, err := os.Getwd()
	if err == nil {
		for {
			candidate := filepath.Join(dir, "renderers", "react-shadcn", "package.json")
			if info, err := os.Stat(candidate); err == nil && !info.IsDir() {
				return filepath.Join(dir, "renderers", "react-shadcn"), nil
			}
	...
	if err == nil {
		...
		for i := 0; i < 5; i++ {
			candidate := filepath.Join(dir, "renderers", "react-shadcn")
	...
	return "", fmt.Errorf("cannot find renderers/react-shadcn/ directory with package.json (walked up from CWD and tried executable path)")
}

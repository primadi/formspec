...
from .ctx import Ctx, FormSpecError, SidecarClient
...
class App:
    """The lib-formspec-python listener.

    Usage::

        app = lib_formspec.App()  # sockets from FORMA_APP_SOCKET / FORMA_SIDECAR_SOCKET
    ...
    def handle(self, name: str, handler: Handler) -> None:
        if name in self._handlers:
            raise FormSpecError(f"handler for {name} already registered")
    ...
    def run(self) -> None:
        ...
        if not self._listen.startswith("unix://"):
            raise FormSpecError(
                f"listen {self._listen}: only unix:// is supported by lib-formspec-python"
            )
        ...
        print(f"[lib-formspec-python] listening on {socket_path}", file=sys.stderr)
        try:

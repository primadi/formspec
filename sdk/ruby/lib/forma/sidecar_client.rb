  ...
  class SidecarClient
    ...
    def initialize(endpoint = nil, timeout: 30)
      ...
      if endpoint.start_with?("unix://")
        ...
        raise FormaError, "sidecar endpoint #{endpoint}: unsupported scheme (want unix:// or http://)"
      end
    end
    ...
    def parse_http_response(sock, path)
      ...
      raise FormaError, "empty response from sidecar" if status_line.nil? || status_line.empty?
      ...
      unless status_code == 200
        ...
        raise FormaError, "sidecar call #{path}: #{error_msg}"
      end

      decoded.is_a?(Hash) ? decoded : {}
    rescue JSON::ParserError
      raise FormaError, "sidecar call #{path}: invalid JSON response"
    end
  end
end

...
module Forma
  # The lib-forma-ruby listener: accepts POST /invoke/{module}/{entity}/{action}
  ...
  class App
    ...
    def handle(action, &block)
      if @handlers.key?(action)
        raise FormaError, "handler for #{action} already registered"
      end
    ...
    def run
      unless @listen.start_with?("unix://")
        raise FormaError, "listen #{@listen}: only unix:// is supported by lib-forma-ruby"
      end
      ...
      $stderr.puts "[lib-forma-ruby] listening on #{socket_path}"

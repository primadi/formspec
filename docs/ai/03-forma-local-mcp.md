# `forma-local-mcp` — Grounding Project Lokal

**Status:** Design — belum diimplementasikan
**License:** Creative Commons CC0

> MCP server lokal, diekspos lewat subcommand `forma mcp-serve` (stdio).
...
| `list_installed_modules()` | Modul di `modules/`+`vendors/`, status aktif/nonaktif dari `forma.lock` ([`../spec/platform/08-project-layout.md`](../spec/platform/08-project-layout.md) §6), termasuk `ai_index` modul terinstal |
| `read_module_spec(module, kind, name)` | Detail satu spec |
| `propose_spec_file(path, content)` | Tulis draft ke sesi + validasi otomatis (§2) |
| `apply_draft(session, file)` | Pindahkan draft yang di-accept ke lokasi asli; guard `vendors/` (§4); auto-backup ke `undo/` ([`02-forma-consult.md`](02-forma-consult.md) §4) |
| `validate_spec(yaml)` | Validasi statis — reuse package boot `forma-server` (§3) |
| `check_naming_conflict(name)` | Deteksi bentrok nama module/entity |
| `restart_server()` / `get_server_status()` / `stop_server()` | Kontrol proses `forma dev` lokal (§5) |
| `list_skills()` / `read_skill(name)` | Index dan isi Forma Skill ([`06-forma-skill.md`](06-forma-skill.md)) |

Setiap tool adalah pembungkus terstruktur di atas `forma-core` — tidak membawa
logic baru ([`01-architecture.md`](01-architecture.md) §4).

## 2. Validation Gate — Perilaku Tool, Bukan Kedisiplinan Pemanggil

Client yang dipakai bisa eksternal (Claude Code/Cursor — di luar kontrol Forma),
...
ditulis ([`06-forma-skill.md`](06-forma-skill.md) §3).
...
saat consult tapi gagal saat `forma-server` betulan boot adalah kelas bug
paling susah dilacak. Satu package Go internal (`forma-core`):

```
forma-server              → boot betulan, load spec, serve traffic
forma apply --dry-run     → CLI, load spec, validasi, TANPA serve traffic
forma-local-mcp           → fungsi validasi yang SAMA, in-process
...
validasi yang butuh instance `forma-server` + DB jalan (mis. "apakah
...
`forma dev` lokal dari dalam sesi. `restart_server()` adalah composite yang
...
| [`02-forma-consult.md`](02-forma-consult.md) | Client yang memanggil tool ini; sesi, diff, undo |
| [`04-forma-remote-mcp.md`](04-forma-remote-mcp.md) | Server saudara untuk data ekosistem |
| [`../spec/platform/08-project-layout.md`](../spec/platform/08-project-layout.md) | Struktur `modules/`/`vendors/`/`overrides/`/`forma.lock` yang dibaca tool di sini |

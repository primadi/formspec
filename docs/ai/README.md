...
structure, not documentation* yang dipegang di seluruh arsitektur FormSpec.

## 1. Komponen

| Komponen | Wujud | Peran |
|---|---|---|
| `formspec-consult` | Binary standalone (TypeScript + Vercel AI SDK, di-compile via `bun build --compile`) | Client mandiri: REPL, tool-use loop, kelola sesi, render diff — [`02-formspec-consult.md`](02-formspec-consult.md) |
...
| LLM Provider Layer | Vercel AI SDK di dalam `formspec-consult` | BYOK multi-provider, tool-use loop, normalisasi format tool-call — [`05-llm-provider-layer.md`](05-llm-provider-layer.md) |

...
**Versi:** 1.9
**Status:** Final — seluruh Decisions Log (D1–D35) disetujui
...
| D18 | **Kind extensible tiga lapis**: bawaan spec (`App, Module, Entity, Service, Config, Migration, Subscription`) → modul resmi (`Seed, Schedule, MailTemplate, ...` via `KindDefinition`, ala CRD) → modul pihak ketiga (ter-namespace, tunduk Verified Badge). Guardrail: developer aplikasi hampir tidak pernah membuat kind — butuh kind baru = memperluas framework; 95% kasus jawabannya `Entity` | **Final** |
...
| Q8 | Spesifikasi detail fitur adopsi Frappe (D10): sintaks form layout hints, print format, approval-in-state-machine |
| Q9 | Spesifikasi detail realtime subscription per-entity (D10): konvensi channel, permission, filter |
| Q10 | Scope OPA: apakah deployment policy environment ditulis 100% Rego, atau YAML policy sederhana + Rego untuk kasus kompleks |
...
| Q12 | Katalog kind frontend (Page, Form, Table, Dashboard, Menu, ...?) + kontrak escape hatch custom component (`asset`): format bundle, props, lifecycle |
| Q13 | Desain REPL: scope akses per environment (dev: penuh; production: read-only default?), audit trail sesi REPL |
| Q14 | Mekanisme `KindDefinition` (Extended): format JSON Schema untuk validasi spec, lifecycle handler, aturan namespace kind pihak ketiga |
...
| Q16 | Formula revenue sharing & metering modul (D22): per-install, per-tenant, per-usage? Penyelesaian ke vendor modul |
...
| # | Concern | Kind | Letak |
|---|---|---|---|
| 1 | Packaging & deployment | `App`, `Module` | Core Basic |
...
| **Data Owner** | Pemilik workspace: data, users, grants, compute, billing |
| **App Owner** | Vendor aplikasi: artifact, versi, konten seed reference; tanpa akses data production |
| **Module Vendor** | Penerbit modul di marketplace (bisa ≠ App Owner) |
| **Provider App** | App yang tujuannya mem-publish service ke app lain; vendornya = Data Owner atas workspace-nya sendiri; menggantikan konsep "data global" |
| **Grant** | Izin akses lintas-app: di-request konsumen, disetujui Data Owner penyedia, tercatat, revocable; titik metering revenue sharing |
| **Platform Operator** | Yang menjalankan Control Plane + infra: Forma Cloud, atau pemegang enterprise license yang menjalankan Forma Cloud sendiri |

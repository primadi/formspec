...
```bash
# macOS / Linux
curl -fsSL https://formspec.dev/install.sh | sh -s -- --version v0.0.7
# atau via env: FORMSPEC_VERSION=v0.0.7 sh -c "$(curl -fsSL https://formspec.dev/install.sh)"
```

```powershell
# Windows
$env:FORMSPEC_VERSION = 'v0.0.7'; irm https://formspec.dev/install.ps1 | iex
```
...
curl -fsSL -o formspec.tar.gz \
  https://github.com/primadi/formspec/releases/download/v0.0.7/formspec-darwin-arm64.tar.gz

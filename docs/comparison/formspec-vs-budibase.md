...
| **Database**                 | PostgreSQL + Valkey + Garage — raw SQL via `ctx.db`                                        | Built-in (CouchDB/internal) or connect to Postgres/MySQL/SQL Server            | **Any SQL** (MySQL, Postgres, MSSQL, SQLite, MariaDB) — connects to your existing database |

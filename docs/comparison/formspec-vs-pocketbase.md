...
A complete ecosystem for business applications. YAML manifests define entities, state machines, actions, permissions, and UI events. Two-process architecture (Control + Resource Planes). Five implementation types (Go native, Starlark, compiled, sidecar). PostgreSQL + Valkey + Garage. Module marketplace with pricing and governance.
...
| File storage             | ✅ `ctx.storage` (Garage/MinIO/S3)   | PocketBase uses local FS + S3              |
...
| **File Storage**             | ✅ `ctx.storage` (Garage/MinIO/S3)                                     | ✅ Built-in (local FS + S3)                                             |
...
| **Production Scale**         | Enterprise (PostgreSQL + Valkey + Garage)                              | Small-to-medium (SQLite limits)                                         |

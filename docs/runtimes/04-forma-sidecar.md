...
Pod
┌────────────────────────────┐      ┌───────────────────────────────┐
│ Container: forma-sidecar   │      │ Container: app                 │
│  - forma-resource engine   │      │  - app.php (business logic)    │
│    (compiled-in, sama      │◄────►│  - lib-forma-php (SDK tipis)   │
...
**Yang penting:** traffic REST API end-user masuk **ke `forma-sidecar`**, bukan ke app.php. `forma-sidecar` men-generate & serve seluruh route CRUD (persis seperti app Go native). Hanya saat action punya `impl.type: sidecar`, `forma-sidecar` **memanggil keluar** ke proses app untuk eksekusi business logic — lalu app **memanggil balik** ke `forma-sidecar` untuk `ctx.*` primitives (db, cache, lock, dst) selama eksekusi itu.
...
| Fitur | Deskripsi |
|---|---|
| **Embed Forma Resource penuh** | Entity engine, REST API generator, permission enforcement — identik dengan app Go native (lihat `02-forma-resource.md`) |
...
### 3.2 Package yang Dipakai Ulang dari Forma Resource

`forma-sidecar` tidak mengimplementasikan ulang entity engine — ia meng-compile-in package Go yang **sama persis** dengan yang dipakai app Go native (`internal/entity`, `internal/api`, `internal/permission`, `internal/db`, `internal/auth`). Bagian yang **ditambahkan khusus untuk sidecar**:
...
`lib-forma-php` (dan sejenisnya) mengekspos `ctx.*` sebagai objek native di bahasa itu, tapi setiap panggilan sebenarnya adalah HTTP call balik ke sidecar — memakai **kontrak primitive yang sama** dengan `internal/starlark/primitive.go` (`query`/`get`/`set`/`delete`/`acquire`/`release`), supaya Starlark dan app non-Go berbagi implementasi backend yang identik:
...
3. Serialize/deserialize tipe data Forma (Document, Field types) ke/dari tipe native bahasa tersebut

**Tidak ada logic bisnis Forma** (state machine, permission, entity storage) di sisi `lib-forma-*` — semuanya di `forma-sidecar` (keputusan desain, lihat diskusi awal dokumen ini).
...
# Mode child-process — forma-sidecar meng-exec app.php sendiri
forma-sidecar \
  --runtime php:8.3 \
  --app-dir /srv/app \
...
Referensi lengkap ada di `--help` (`cmd/forma-sidecar/main.go`); ringkasan flag yang relevan dengan §5: `--runtime` (`local` default, atau `php`/`python`/`node`), `--app-dir` (default `{state-dir}/app`), `--app-entrypoint` (default per-runtime: `app.php`/`app.py`/`app.js`).

---

## 7. Health Aggregation

`GET /health` di `forma-sidecar` merefleksikan dua hal:
1. Kesehatan proses sidecar sendiri (entity engine, DB connection)
2. Kesehatan proses app — sidecar melakukan ping berkala ke `lib-forma-*` listener; kalau app tidak merespons N kali berturut-turut, `/health` sidecar melaporkan `degraded` (bukan `unhealthy` — sidecar sendiri masih bisa serve request yang tidak butuh handler app, mis. CRUD murni tanpa custom action).

---

## 8. Status Implementasi Hari Ini

**Implementasi awal sudah ada.** `cmd/forma-sidecar` + `internal/sidecar` mengimplementasikan:
...
- Startup pull artifact (§3.1) — reuse `internal/resource` deployer; artifact envelope memisah file YAML (→ `--state-dir/spec`, jadi input `resource.New`) dari file lain seperti source app (→ `--app-dir`), sesuai desain §3.1 ("YAML specs + source code app + runtime info"). Artifact baru setelah boot ditulis ke disk + log `restart required` (hot-rebuild route belum ada — sama dengan gap `SyncAgent` di `02-forma-resource.md` §7).
- Mode eksekusi app (§5) — **kedua mode sudah ada**: `--runtime local` (default, container terpisah) dan `--runtime php|python|node` (child-process, `cmd/forma-sidecar/childprocess.go`) yang meng-exec interpreter dari `--app-dir`/`--app-entrypoint` dengan env `FORMA_APP_SOCKET`/`FORMA_SIDECAR_SOCKET` otomatis, plus shutdown graceful (SIGTERM lalu SIGKILL setelah timeout).
...
2. Bangun `lib-forma-php` sebagai proof-of-concept pertama (PHP dipilih karena paling umum untuk kasus penggunaan Forma) — listener `/invoke` minimal + client `ctx.db`/`ctx.lock` yang manggil §4.3.
3. Selesaikan dulu gap `ctx.*` di `02-forma-resource.md` §7 (`SetDatastoreResolver` belum di-wire) — endpoint `/ctx/db/query` di sidecar pada akhirnya delegasi ke kode yang sama, jadi tidak ada gunanya membangun proxy-nya sebelum backend-nya nyata.
4. Tentukan mode eksekusi proses app (§5) — perlu diputuskan sebelum tooling `forma apply`/image builder untuk app non-Go dibangun.

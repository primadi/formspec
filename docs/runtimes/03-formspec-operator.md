# FormSpec Operator — Binary Reference
...
> `formspec-operator` adalah **CRD controller** yang berjalan sebagai pod di setiap K8s cluster. Ia menerjemahkan CRD FormSpec (`Workspace`, `Datastore`, `ResourceClaim`, `ClusterClass`) menjadi resource K8s native (Deployment, Service, Secret, ConfigMap, HPA). **Satu-satunya komponen closed source** di FormSpec (D-ARCH-15). Dokumen ini fokus pada binary-nya sendiri — desain internal, konfigurasi, dan API/kontrak komunikasinya. Untuk gambaran CRD dan topologi K8s secara umum, lihat `docs/architecture/06-k8s-operator.md`.
...
Workspace Created/Updated:
  ...
  2. Hitung desired Deployment spec:
     - image: formahub/formspec-resource:<version-pinned>

...
- **`ai_skills/schema-validation/SKILL.md`** (baru): instruksi menjalankan
  `forma validate`, membaca output dua lapis (`engine:` vs `schema:`), katalog
...
menangkap folder baru (diverifikasi `go build ./cmd/forma`).

## Kenapa

Usai `forma validate` diimplementasikan (changelog 005), belum ada skill yang
...
ikut di-distribusikan ke project hasil `forma init`.
...
- `cmd/forma/validate.go`, `docs/changelog/2026-07-31-005-*.md`

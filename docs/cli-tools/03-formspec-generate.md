...
Nothing is silently guessed: if an entity has no `expose:` block, it produces zero routes and zero generated code for it (deny-by-default, D49) — `formspec generate` errors out if *no* entity is exposed at all, since there'd be nothing to generate.

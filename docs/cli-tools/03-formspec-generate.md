...
| `money`                              | `{ amount: string; currency: string }`                                                    | First-class `{amount, currency}` pair (`05-field-types.md` §2). `amount` is arbitrary-precision decimal → **string**, never `number`.                                                                                                                                           |
| `file` / `attachment`                | `{ key: string; filename: string; content_type: string; size: number; checksum: string }` | Pointer to a `ctx.storage` object with canonical metadata (`05-field-types.md` §1.3). `attachment` is normalized to `file` at validate time.                                                                                                                                    |

...
*governable* — bukan sekadar telemetry.
...
sha256 + versi per workspace), `trust` (public key owner/vendor/platform),
`grants`, `licenses`, `revocations`, `memberships`.
...
mengganggu asimetri pull-only: push-hint hanya memberi tahu *ada snapshot baru*,

...
FR1, FR3, FR4 paling kritis — salah di sini berarti uang salah hitung atau jurnal hilang.

---

## 2. Dari FR ke Konstruksi Forma

### 2.1 Struktur project (tiga jenis file — Foundation D14)
...
tokoku/
  ...
  modules/
    ...
    notifications/
      module.yaml
      subscriptions/wa-on-order-paid.yaml   # kind: Subscription (§3.7)
    gl/
      module.yaml
...
### 2.2 Setiap FR punya rumah yang spesifik
...
## 3. Skenario B — Dengan Forma Core Basic v0.2.0
...
spec:
  ...
  actions:
    ...
    - name: mark-paid
      description: Transisi ke paid — dipanggil oleh payment-gateway.webhook (§3.4)
      ...
      # Handler-nya kini 3 baris (§3.4) — semua konsekuensi pembayaran pindah
  ...
  events:
    - name: paid
      ...
      deliver:
        ...
        # Rumahnya kind: Subscription di modul notifications (§3.7, D35).
...
Perhatikan blok `deliver` ini sekarang adalah **peta lengkap konsekuensi pembayaran** — empat kelas jaminan berbeda terbaca dalam satu tempat: audit, realtime-boleh-hilang, background-reliable, dan transaksional.

Semua konsekuensi pembayaran kini terdeklarasi di satu tempat: auth wajib default + `required_permission` per action (tanpa auth = mustahil, bukan terlupakan) · nomor order otomatis per-tenant (counter `forma_natural_key_counters` ber-key tenant — lock tidak pernah global lintas tenant) · validasi item/total = guard state machine + field rules · audit = `audit: true` · hilangnya event = mustahil karena `mark-paid` mengubah status DAN menulis outbox dalam satu transaksi DB. **Yang jujur belum otomatis:** verifikasi signature webhook — rumahnya `kind: Webhook` (Extended, strategi hmac/rsa).

### 3.3 Script `checkout` (FR1, FR2) — script_ref, editable dari admin panel
...
def execute(resource, params, ctx):
    ...
    # blok `conditions` action (lihat §3.2) sehingga terbaca di kontrak.
...
### 3.4 Alur webhook yang benar: `payment-gateway.webhook` → `order.mark-paid` → `deliver`
...
**Lapis 1 — penerima webhook** (action di Service `payment-gateway`, §3.5):
...
**Lapis 3 — semua konsekuensi = blok `deliver`** (lihat §3.2): jurnal
...
### 3.5 Integrasi eksternal = Service, tidak ada jalur lain
...
### 3.6 Sisi penerima: `gl.journal-entry`
...
### 3.7 Reaksi yang lahir belakangan: `kind: Subscription` (D35)
...
## 4. Pemetaan ke Primitif (closed set — semua terpakai natural)

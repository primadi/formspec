// ─── FormSpecExpr Unified API ───
//
// High-level convenience API for the FormSpecExpr interpreter.
...
/**
 * Evaluate a FormSpecExpr expression string against the given context.
 *
 ...
 * @example
 * ```ts
 * evalFormSpecExpr('fields.status == "paid"', { fields: { status: "paid" } })
 * // → { value: true, warnings: [], valid: true }
 * ```
 */
export function evalFormSpecExpr(
  expr: string,
  context: EvalContext = {},
): FormSpecExprResult {
  if (!expr || expr.trim() === "") {
...
/**
 * Validate a FormSpecExpr expression string (syntax check only).
 * Returns `{ valid: true }` or `{ valid: false, error: "..." }`.
 */
export function validateFormSpecExpr(expr: string): ValidationResult {
  if (!expr || expr.trim() === "") {
    return { valid: true }
  }
...
// ── Convenience wrappers for common FormSpecExpr use-cases ──
...
export function evalVisibleWhen(
  ...
  if (!expr) return true
  const result = evalFormSpecExpr(expr, context)
